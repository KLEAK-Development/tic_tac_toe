...
class TwoPlayerGameScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(l10n.twoPlayerGame)),
      body: SafeArea(
...
class _GameBoard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    ref.listen(twoPlayerGameProvider, (previous, next) {
        ...
        child: GridView.builder(
          ...
          itemBuilder: (context, index) {
            return _GameCell(
              ...
              onTap: () =>
                  ref.read(twoPlayerGameProvider.notifier).makeMove(index),
            );
          },
        ),
      ),
...
class _ResetButton extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    void Function()? onPressed =
        ref.watch(twoPlayerGameProvider).board.hasNotStarted
        ? null
        : () => ref.read(twoPlayerGameProvider.notifier).resetGame();
...
class _GameOverDialog extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final status = ref.read(twoPlayerGameProvider).status;
    final l10n = context.l10n;
